...
def validate_dataframe(df: pd.DataFrame, required_columns: list[str] | None = None) -> pd.DataFrame:
    if required_columns:
...
def calculate_statistics(df: pd.DataFrame) -> dict:
    ...
    for col in numeric.columns:
        ...
        skew = float(s.skew())
        skew_label = "Symmetric ✓" if abs(skew) < 0.5 else "Right-skewed →" if skew > 0 else "Left-skewed ←"
...
def _pick(df: pd.DataFrame, x_col: str | None, y_col: str | None) -> tuple[str, str]:
    all_cols = list(df.columns)
    num_cols = list(df.select_dtypes(include="number").columns)
    if not x_col or x_col not in all_cols:
        x_col = all_cols[0]
    if not y_col or y_col not in all_cols:
        y_col = num_cols[0] if num_cols else all_cols[min(1, len(all_cols) - 1)]
    return x_col, y_col
...
def generate_plotly_chart(df, chart_type, x_col=None, y_col=None):
    ...
    num_cols = list(df.select_dtypes(include="number").columns)
    if chart_type == "bar":
        fig = px.bar(df, x=x_col, y=y_col, title=f"Bar — {y_col} by {x_col}", color_discrete_sequence=COLORS, text_auto=".2s")
        fig.update_traces(textposition="outside", marker_line_width=0)
        fig.update_layout(**PLOTLY_BASE)
    elif chart_type == "line":
        fig = px.line(df, x=x_col, y=y_col, title=f"Line — {y_col} over {x_col}", color_discrete_sequence=COLORS, markers=True)
        fig.update_traces(line_width=2.5, marker_size=6)
        fig.update_layout(**PLOTLY_BASE)
    elif chart_type == "scatter":
        kw = {"trendline": "ols"} if pd.api.types.is_numeric_dtype(df[x_col]) else {}
        fig = px.scatter(df, x=x_col, y=y_col, title=f"Scatter — {x_col} vs {y_col}", color_discrete_sequence=COLORS, opacity=0.72, **kw)
        ...
        fig = px.pie(vc, names="label", values="value", title=f"Pie — {src} Distribution", color_discrete_sequence=COLORS, hole=0.35)
        ...
        for i, c in enumerate(cols):
            fig.add_trace(go.Box(y=df[c], name=c, marker_color=COLORS[i % len(COLORS)], boxmean="sd", jitter=0.35, pointpos=-1.6, marker_size=3))
        fig.update_layout(title="Box Plot — Numeric Distributions", showlegend=False, **PLOTLY_BASE)
    elif chart_type == "histogram":
        fig = px.histogram(df, x=y_col, nbins=35, title=f"Histogram — {y_col}", color_discrete_sequence=COLORS, marginal="box", opacity=0.85)
        fig.update_layout(**PLOTLY_BASE)
    elif chart_type == "area":
        fig = px.area(df, x=x_col, y=y_col, title=f"Area — {y_col} over {x_col}", color_discrete_sequence=COLORS)
        fig.update_layout(**PLOTLY_BASE)
    elif chart_type == "heatmap":
        if len(num_cols) < 2:
            raise ValueError("Heatmap requires at least two numeric columns.")
        corr = df[num_cols].corr().round(2)
        fig = px.imshow(corr, text_auto=True, title="Correlation Heatmap", color_continuous_scale="RdBu_r", zmin=-1, zmax=1, aspect="auto")
        base = {k: v for k, v in PLOTLY_BASE.items() if k not in ("xaxis", "yaxis")}
        fig.update_layout(**base)
        ...
        fig = go.Figure()
        for i, c in enumerate(cols):
            fig.add_trace(go.Violin(y=df[c], name=c, box_visible=True, meanline_visible=True, fillcolor=COLORS[i % len(COLORS)], line_color=COLORS[i % len(COLORS)], opacity=0.72))
        fig.update_layout(title="Violin Plot — Distributions", showlegend=False, **PLOTLY_BASE)
    else:
        raise ValueError(f"Unknown chart type: {chart_type}")
    return pio.to_html(fig, full_html=False, include_plotlyjs="cdn", config={"responsive": True, "displayModeBar": True, "toImageButtonOptions": {"format": "png", "scale": 2}})
...
def generate_matplotlib_chart(df, chart_type, x_col=None, y_col=None):
    ...
    with plt.rc_context(MPL_RC):
        if chart_type == "bar":
            ...
            fig, ax = plt.subplots(figsize=(12, 5))
            ax.plot(df[x_col], df[y_col], color=COLORS[0], lw=2.5, marker="o", ms=5, mfc="white", mew=2)
            ...
            ax.scatter(df[x_col], df[y_col], color=COLORS[0], alpha=0.65, edgecolors="white", lw=0.5, s=65)
            if pd.api.types.is_numeric_dtype(df[x_col]) and HAS_SCIPY:
                ...
                ax.plot(xs, m * xs + b, color=COLORS[2], lw=2, ls="--", label=f"Trend  R²={r**2:.3f}")
                ax.legend()
            ax.set_xlabel(x_col)
            ax.set_ylabel(y_col)
            ...
            ax.pie(vc.values, labels=vc.index, autopct="%1.1f%%", colors=COLORS[: len(vc)], startangle=140, pctdistance=0.82, wedgeprops=dict(width=0.6, edgecolor="white", lw=2))
            ax.set_title(f"Pie — {src}")
        elif chart_type == "box":
            cols = num_cols[:10]
            fig, ax = plt.subplots(figsize=(max(8, len(cols) * 1.8), 6))
            bp = ax.boxplot([df[c].dropna() for c in cols], patch_artist=True, medianprops=dict(color="white", lw=2.5), whiskerprops=dict(lw=1.5), capprops=dict(lw=1.5), flierprops=dict(marker="o", ms=4, alpha=0.5))
            ...
            ax.hist(df[y_col].dropna(), bins=30, color=COLORS[0], edgecolor="white", lw=0.4, alpha=0.85)
            mu = df[y_col].mean()
            sd = df[y_col].std()
            ...
            for i in range(n):
                for j in range(n):
                    ax.text(j, i, f"{corr.iloc[i, j]:.2f}", ha="center", va="center", fontsize=8, color="white" if abs(corr.iloc[i, j]) > 0.5 else "#1e293b")
            ax.set_title("Correlation Heatmap")
        elif chart_type == "violin":
            cols = num_cols[:8]
            fig, ax = plt.subplots(figsize=(max(8, len(cols) * 2), 6))
            parts = ax.violinplot([df[c].dropna() for c in cols], positions=range(1, len(cols) + 1), showmeans=True, showmedians=True)
